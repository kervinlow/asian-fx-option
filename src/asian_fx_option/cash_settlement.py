...
from .exceptions import MissingFinalRateError, MissingSettlementFixingError, ZeroAverageRateError
...
def expected_cash_flow(
    ...
    if spec.is_floating_strike:
        if spec.final_rate_raw is None:
            raise MissingFinalRateError(
                "final_rate_raw must be provided when is_floating_strike=True"
            )
        underlying = normalize_rate(spec.final_rate_raw, spec.quotation_type, spec.pair_scaling)
    else:
        underlying = s_avg

    if spec.option_type == OptionType.CALL:
        payoff_per_base = max(0.0, underlying - k_norm)
    else:
        payoff_per_base = max(0.0, k_norm - underlying)
